...
export default function About() {
  ...
  const [endi,setendi] = useState('Enable dark mode');
  const [Dark,setDark] = useState(Darkobj)
  const DarkMode = () =>{
    if(Dark.color==='black'){
        setDark({
          color : 'white',
        ...
        setendi("Enable light mode");

      }
      else{
        setDark(Darkobj);
        setendi("Enable dark mode");
...
<button className="btn btn-primary my-3" onClick={DarkMode}>{endi}</button>
    </div>
  )
}
